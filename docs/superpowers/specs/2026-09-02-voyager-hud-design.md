...
- **Grab manager** — polls modifier state via direct `CGEventSourceFlagsState` FFI call at ~10 Hz. While the configured combo (default ⌘⌥) is held: `set_ignore_cursor_events(false)` and emits `grab-mode { on: true }`; on release, back to click-through. Default is ⌘⌥, not plain ⌘, so ordinary ⌘-shortcuts never make the overlay swallow a click. Zero extra crates and, unlike key-state polling, requires no Input Monitoring permission.
